...
export class RxdbShadowDocSource implements DocSource {
    ...
    async push(docId: string, currentUpdate: Uint8Array): Promise<void> {
        ...
        try {
            ...
            if (!debounceUpdate) {
                debounceUpdate = debounce(async (doc: RxDocument<Entry>, latest: UpdateMessage) => {
                    await this._uploadContent(doc, Uint8Array.from(latest.update));
                    await this._updateRemote(doc, Uint8Array.from(latest.update));
                    await doc.update({
                        $set: {
                            contentTimestamp: latest.timestamp,
                        }
                    });
                }, 1000);
            ...
            await debounceUpdate(doc, content);
        } catch (error) {
